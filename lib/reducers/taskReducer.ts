...
const applyFilters = (state: TaskState) => {
  ...
  if (state.selectedCategory) {
    result = result.filter((task) => task.categoryId === state.selectedCategory)
  }
  ...
  switch (action.type) {
    ...
    case actions.DELETE_TASK_SUCCESS:
      ...
      return {
        ...
        deletingTaskIds: state.deletingTaskIds.filter((id) => id !== action.payload),
      }
    case actions.DELETE_TASK_FAILURE:
      return {
        ...state,
        error: action.payload.error,
        deletingTaskIds: state.deletingTaskIds.filter((id) => id !== action.payload.id),
      }
    ...
    case actions.UPDATE_TASK_SUCCESS:
      const tasksAfterUpdate = state.tasks.map((task) => (task.id === action.payload.id ? action.payload : task))
      return {
        ...state,
    ...
    case actions.SET_SELECTED_CATEGORY:
      return {
        ...
        filteredTasks: applyFilters({ ...state, selectedCategory: action.payload }),
      }
